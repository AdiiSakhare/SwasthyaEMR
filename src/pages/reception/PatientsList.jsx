  ...
  return (
    <div className="max-w-6xl mx-auto fade-up-enter">
      <PageHeader
        eyebrow="Patients"
        title="All Patients"
        description={`${patients.length} registered patients in your clinic.`}
        actions={
      ...
      <div className="bg-card rounded-2xl border border-border overflow-hidden">
        ...
        {filtered.length === 0 ? (
          <Empty
            ...
            description={q ? "Try a different search term, or register a new patient." : "Register your first patient to begin."}
            action={
              <Button onClick={() => navigate("/reception/patients/new")}>
                <UserPlus className="size-4" />
                Register Patient
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-left text-xs font-semibold text-muted-foreground px-5 py-3 uppercase tracking-wider hidden md:table-cell">Phone</th>
                ...
                {filtered.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 text-sm text-muted-foreground hidden md:table-cell">{p.phone}</td>
                      <td className="px-5 py-3.5 hidden md:table-cell">
                        <div className="flex flex-wrap gap-1">
                          {p.conditions.length === 0 ? (
                            <span className="text-xs text-muted-foreground">—</span>
                          ) : (
                            p.conditions.slice(0, 2).map((c) => <Badge key={c} variant="default">{c}</Badge>)
                          )}
                        </div>
                      </td>
